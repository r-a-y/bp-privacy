...
/**
 * Friends Privacy Settings Screen
 *
...
function bp_authz_add_friends_nav() {
	global $bp;
	...
	if( bp_privacy_filtering_active( 'friends' ) ) {
		bp_core_new_subnav_item( array( 'name' => __( 'Friends Privacy', BP_AUTHZ_PLUGIN_NAME ), 'slug' => 'friends-privacy', 'parent_url' => $privacy_link, 'parent_slug' => $bp->authz->slug, 'screen_function' => 'bp_authz_screen_friends_privacy', 'position' => 30, 'user_has_access' => bp_is_my_profile() ) );
	};
}
add_action( 'bp_authz_add_settings_nav', 'bp_authz_add_friends_nav' );


function bp_authz_screen_friends_privacy() {
	global $bp_privacy_updated, $privacy_form_error;
	
	$bp_privacy_updated = false;
	$privacy_form_error = false;
	
	if ( isset( $_POST[ 'bp-authz-friends-submit' ] ) && isset( $_POST[ 'bp-authz' ] ) ) {
		if ( !check_admin_referer( 'bp-authz-privacy-friends', '_wpnonce_privacy-friends' ) )
			return false;
	...
	do_action( 'bp_authz_friends_privacy' );

	add_action( 'bp_template_content', 'bp_authz_screen_friends_privacy_content' );
	
	bp_core_load_template( apply_filters( 'bp_core_template_plugin', 'members/single/plugins' ) );
}

function bp_authz_screen_friends_privacy_content() {
	...
	<form action="<?php echo $bp->loggedin_user->domain . 'privacy/friends-privacy' ?>" method="post" id="privacy-settings-form">
		<h3><?php _e( 'Set the rights to who can see your friends list or request friendship', BP_AUTHZ_PLUGIN_NAME ) ?></h3>
		...
		<?php wp_nonce_field( 'bp-authz-privacy-friends', '_wpnonce_privacy-friends' ); 
		...
		// Eventually replace below two queries with a single call to bp_authz_retrieve_user_acl_recordset()
		
		?>
								
		<table class="privacy-settings" id="friends-privacy-settings">
			
			<thead>
				<tr>
					<th class="title"><?php _e( 'Friends Actions', BP_AUTHZ_PLUGIN_NAME ) ?></th>
			...
			<tbody>
				<tr>
					<td class="title"><?php _e( 'Who can view your friends List', BP_AUTHZ_PLUGIN_NAME ) ?></td>
					
						<?php // this first selection box is for setting viewing rights to a user's friends list 
					...
					<td class="userlist">
						 ...
						 */
						?>							
						<div id="friends-acl-<?php echo $acl_rec; ?>-json-lists-<?php echo $acl_rec; ?>">
						...
						<div id="friends-acl-<?php echo $acl_rec; ?>-listbox">
				...
				<tr>
					<td class="title"><?php _e( 'Who can request friendship', BP_AUTHZ_PLUGIN_NAME ) ?></td>
	
					<?php // This second selection box is for setting who can request friendship via the "Add Friend" button 
						...
						Array before alteration:
							0 => 'All Users' *** remove element: non-logged in users cannot send friend requests
							1 => 'Logged in Users'
							2 => 'Friends' *** remove element: friends are already friends
							3 => 'Members of these Groups'
							4 => 'These Users Only',
							5 => 'Only Me' *** change this to read "No One"
						
						The result is that only four elements are available in the selection box. Of
						...
						*/
						...
						unset( $bp->authz->bpaz_acl_levels[ 'Friends' ] );
		...
		<?php do_action( 'bp_authz_friends_privacy_content' ) ?>
		
		<p class="submit"><input type="submit" name="bp-authz-friends-submit" value="<?php esc_attr_e( 'Save Changes', BP_AUTHZ_PLUGIN_NAME ) ?>" id="submit" class="auto"/></p>		
